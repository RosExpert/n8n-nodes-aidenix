...
- Rewrote README for the n8n verified-community-node submission: added
  value proposition, prerequisites, quick start, common workflow patterns,
  error-handling reference, troubleshooting, and compatibility notes.
- Added in-app screenshots (`docs/screenshot1.png`, `docs/screenshot2.png`)
  and shipped them in the npm tarball via the `files` field.
- Migrated `tsconfig.json` to `module`/`moduleResolution: "node16"` to
  remove the TypeScript 5.5 deprecation warning; enabled `isolatedModules`
